...
sat_map_layer = 'TRUE_COLOR' # TODO: Sure we don't want also other bands?
max_clouds = 0.5 

# Spatial dimensions of each of the images to download
# (EPSG 3857 is measured in meters)
box_height = 2560 
box_width  = 2560 # (Optimal value is img_size * lowest_resolution(bands))

# Dimensions of the downloaded images
img_size = (256, 256)

# Initialize me to bottom left and go to top right
bottom_left = to_epsg3857((52.07655197641196, 4.796562194824219))
top_right   = to_epsg3857((52.18445786400999, 5.07122039794922))
...
# Scans the map from bottom left to top right, downloading all images corresponding to each tile
# within the desired timestamps
for i in range(0, int(abs((bottom_left[0]-top_right[0])/box_height))):
    ...
    for v in range(0, int(abs((bottom_left[1]-top_right[1])/box_width))):
        ...
        # Get ground truth from geopedia and store it
        tulip_fields = TulipFieldRequest(bbox=bbox, width=img_size[0], height=img_size[1], crs=3857, layer=layers['tulip_field_2016'])
        ...
        # Get satellite images for the given ROI and timespan from sentinel hub
        s2_request = S2Request(WMS_INSTANCE, layers=sat_map_layer, time=('2016-03-01', '2017-10-20'), bbox=bbox,
                               width=img_size[0], height=img_size[1], crs=3857, maxcc=max_clouds)
        ...
        # Uptade bounding box to process next tile
